...
<head>
    ...
    <title>@yield('title', 'SeaServer — VPS SeaServer')</title>
    <meta name="description" content="@yield('meta', 'Thuê VPS SeaServer tự động')">
...
<body class="text-gray-800 antialiased bg-[#f8fafc] font-sans flex min-h-screen">

    {{-- ===== MOBILE BACKDROP ===== --}}
    <div id="sidebarBackdrop" class="fixed inset-0 bg-gray-900/50 z-40 hidden md:hidden transition-opacity"></div>

    {{-- ===== SIDEBAR (Left) ===== --}}
    <aside id="sidebar" class="fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-gray-200 flex flex-col h-screen transform -translate-x-full md:translate-x-0 transition-transform duration-200 ease-in-out">
        {{-- Brand / Logo --}}
        ...
        <nav class="flex-1 px-3 py-2 space-y-1 overflow-y-auto custom-scroll">
            ...
            <a href="{{ route('notifications.index') }}" 
               ...
               {{ request()->routeIs('notifications*') ? 'bg-cloud-50 text-cloud-700 border-l-2 border-cloud-600 -ml-3 pl-[14px]' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 rounded-md' }}">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="opacity-70 text-indigo-500"><path stroke-linecap="round" stroke-linejoin="round" d="M14.857 17.082a23.848 23.848 0 005.454-1.31A8.967 8.967 0 0118 9.75v-.7V9A6 6 0 006 9v.75a8.967 8.967 0 01-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 01-5.714 0m5.714 0a3 3 0 11-5.714 0" /></svg>
                <span class="text-indigo-700 font-bold">Thông báo (1)</span>
            </a>
            ...
            <a href="{{ route('affiliate.index') }}" 
               class="flex items-center gap-3 px-3 py-2 text-sm font-medium transition-colors
               {{ request()->routeIs('affiliate*') ? 'bg-cloud-50 text-cloud-700 border-l-2 border-cloud-600 -ml-3 pl-[14px]' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 rounded-md' }}">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="opacity-70"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                Affiliate
            </a>
            ...
            <a href="{{ route('reseller.api.index') }}" 
               class="flex items-center gap-3 px-3 py-2 text-sm font-medium transition-colors
               {{ request()->routeIs('reseller.api*') ? 'bg-cloud-50 text-cloud-700 border-l-2 border-cloud-600 -ml-3 pl-[14px]' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 rounded-md' }}">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="opacity-70"><path stroke-linecap="round" stroke-linejoin="round" d="M14.25 9.75L16.5 12l-2.25 2.25m-4.5 0L7.5 12l2.25-2.25M6 20.25h12A2.25 2.25 0 0020.25 18V6A2.25 2.25 0 0018 3.75H6A2.25 2.25 0 003.75 6v12A2.25 2.25 0 006 20.25z" /></svg>
                Reseller (API)
            </a>
            @endif
            @endauth
            ...
            <div class="pt-4 mt-4 border-t border-gray-100">
                ...
                <a href="{{ route('admin.affiliates.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-md text-sm font-medium text-gray-600 hover:bg-gray-50 hover:text-gray-900 {{ request()->routeIs('admin.affiliates.*') ? 'bg-gray-100 text-gray-900' : '' }}">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="opacity-70"><path stroke-linecap="round" stroke-linejoin="round" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" /></svg>
                    QL Affiliate
                </a>
                ...
                <a href="{{ route('admin.proxies.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-md text-sm font-medium text-gray-600 hover:bg-gray-50 hover:text-gray-900 {{ request()->routeIs('admin.proxies.*') ? 'bg-gray-100 text-gray-900' : '' }}">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="opacity-70"><path stroke-linecap="round" stroke-linejoin="round" d="M7.5 6h9m-9 6h9m-9 6h9M4.5 6h.008v.008H4.5V6zm0 6h.008v.008H4.5V12zm0 6h.008v.008H4.5V18z" /></svg>
                    QL Proxy
                </a>
                <a href="{{ route('admin.revenue.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-md text-sm font-medium text-gray-600 hover:bg-gray-50 hover:text-gray-900 {{ request()->routeIs('admin.revenue.*') ? 'bg-gray-100 text-gray-900' : '' }}">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="opacity-70"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                    Doanh thu & Lãi
                </a>
        ...
        <div class="p-3 border-t border-gray-100 space-y-1">
            <a href="{{ route('support.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-md text-sm font-medium text-gray-600 hover:bg-gray-50 hover:text-gray-900 {{ request()->routeIs('support.*') ? 'bg-gray-100 text-gray-900' : '' }}">
    ...
    <div class="flex-1 flex flex-col min-w-0 min-h-screen md:ml-64">
        ...
        <main class="flex-1 custom-scroll">
    ...
    <div id="globalFullScreenLoader" class="fixed inset-0 z-[110] flex-col items-center justify-center bg-white/80 backdrop-blur-sm hidden">
        <div class="text-center bg-white p-8 rounded-xl shadow-2xl border border-gray-100 max-w-sm w-full mx-4 transform transition-all">
            <div class="w-16 h-16 mx-auto mb-5 rounded-full border-4 border-cloud-100 border-t-cloud-600 animate-spin"></div>
            <h3 class="text-xl font-bold text-gray-900 mb-2">Đang xử lý yêu cầu...</h3>
            <p class="text-sm text-gray-500">Vui lòng đợi giây lát và không đóng trình duyệt lúc này.</p>
        </div>
    </div>
    ...
    <script>
        ...
        // Global interceptor for form submissions (Confirm + Loading State)
        document.addEventListener('submit', function(e) {
            ...
            if (form && form.hasAttribute('data-confirm')) {
                ...
                showGlobalConfirm(msg, function() {
                    ...
                    setFormLoadingState(form);
                    setTimeout(() => form.submit(), 50);
                });
                return;
            }
            
            // If no confirm, prevent default, set state and submit
            e.preventDefault();
            setFormLoadingState(form);
            setTimeout(() => form.submit(), 50);
        });

        function setFormLoadingState(form) {
            form.dataset.submitted = 'true';
            
            // Hiện hiệu ứng Full-Screen Loader cho an toàn tuyệt đối
            const loader = document.getElementById('globalFullScreenLoader');
            if (loader) {
            ...
            if (btn) {
                // Prevent width jumping if possible
